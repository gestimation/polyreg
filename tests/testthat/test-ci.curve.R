test_that("ci.curve yields the same outputs as cif of mets", {
  ...
  t <- ci.curve(Surv(t, epsilon)~1, data=testdata, report.ggsurvfit = FALSE, conf.type = "none")
...
test_that("ci.curve by strata yields the same outputs as subsetting", {
  ...
  library(Rcpp)
  e <- cif(Event(t,epsilon)~1, data=testdata, cause=1)
  e <- ci.curve(Surv(t, epsilon)~1, data=testdata_f, report.ggsurvfit = FALSE, conf.type = "none")
  expected <- c(e$surv, e$time)
  t <- ci.curve(Surv(t, epsilon)~strata, data=testdata, report.ggsurvfit = FALSE, conf.type = "none")
