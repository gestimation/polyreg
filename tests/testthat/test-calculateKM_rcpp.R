test_that("calculateKM_rcpp and get_surv produced expected KM for survival data", {
  library(Rcpp)
...
test_that("calculateKM_rcpp and get_surv produced expected stratified KM for survival data", {
  library(Rcpp)
  data("diabetes.complications")
  diabetes.complications$d <- as.numeric(diabetes.complications$epsilon==0)
