estimating_equation_ipcw_roptim <- function(
  ...
  alpha_1 <- alpha_beta[1:i_parameter[1]]
...
estimating_equation_ipcw_future_lapply <- function(
    formula,
    data,
    exposure,
    ip.weight,
  ...
  list_CIFs <- future_lapply(seq_len(nrow(x_l)), function(i) {
    tryCatch({
      log_p <- calculatePotentialCIFs_roptim(
        log_p0         = log_p0,
        alpha_tmp_1    = alpha_tmp_1_vec[i],
        beta_tmp_1     = beta_1,
        alpha_tmp_2    = alpha_tmp_2_vec[i],
        beta_tmp_2     = beta_2,
        effect1        = estimand$effect.measure1,
        effect2        = estimand$effect.measure2,
        prob_bound     = prob.bound,
      ...
      warning(sprintf("エラー at i = %d: %s", i, e$message))
      NULL
    })
  }, future.seed = TRUE)

  # NULL を除いてから行列化
  list_CIFs <- Filter(Negate(is.null), list_CIFs)
  potential.CIFs <- do.call(rbind, list_CIFs)
  if (is.null(dim(potential.CIFs))) {
    potential.CIFs <- matrix(potential.CIFs, ncol = 4, byrow = TRUE)
  }
...
estimating_equation_ipcw_parallel <- function(
    formula,
    data,
    exposure,
    ip.weight,
