calculateCIFs <- function(alpha_beta_tmp, x_a, x_l, offset, epsilon,
                          estimand, optim.method, prob.bound, initial.CIFs = NULL) {
  i_parameter <- rep.int(NA_integer_, 7L)
  ...
  x_l <- as.matrix(x_l); storage.mode(x_l) <- "double"
  alpha_tmp_1 <- as.numeric(x_l %*% matrix(alpha_1, ncol = 1L) + offset)
  alpha_tmp_2 <- as.numeric(x_l %*% matrix(alpha_2, ncol = 1L) + offset)
  ...
  p0_row <- clampP(p0_row, prob.bound)
  p0 <- matrix(rep(p0_row, n), nrow = n, byrow = TRUE)
  log_p0 <- log(p0)
  p0_ref <- p0[1:2]; p0_non <- p0[3:4]

  x_a_mat <- as.matrix(x_a)
  if (any(rowSums(x_a_mat) > 1L)) stop("x_a の各行は one-hot（高々1つだけ1）である必要があります。")
  lev <- attr(x_a, "levels"); ref <- attr(x_a, "reference")
  if (is.null(lev) || is.null(ref)) stop("x_a には 'levels' と 'reference' 属性が必要です。")
  nonref_lev <- setdiff(lev, ref)
  K_nonref   <- length(nonref_lev)
  if (K_nonref < 1L) stop("非参照レベルが見つかりません（ncol(x_a)=0）。")

  ## anchor for ref rows
  anchor_name <- estimand$ref_anchor_level %||% nonref_lev[1]
  anchor_idx  <- match(anchor_name, nonref_lev)
  if (is.na(anchor_idx)) stop(sprintf(
    "ref_anchor_level '%s' が非参照レベルに見つかりません。候補: %s",
    as.character(anchor_name), paste(nonref_lev, collapse = ", ")
  ))

  predicted.CIFs <- matrix(NA_real_, nrow = nrow(x_l), ncol = 4)

  ## which level each row uses (0=ref, 1..K_nonref=non-ref level column)
  lvl_idx <- max.col(cbind(0, x_a_mat), ties.method = "first") - 1L
  if (anyNA(lvl_idx)) {
    warning(sprintf("x_a に NA を含む行が %d 件：その行は参照水準として扱います。", sum(is.na(lvl_idx))))
    lvl_idx[is.na(lvl_idx)] <- 0L
  }

  ## cache per (alpha1, alpha2, L)
  cache <- new.env(parent = emptyenv())
  key_of <- function(a1, a2, L) paste0(format(a1, digits = 16), "|", format(a2, digits = 16), "|", L)

  ## initial block helper (n×2 / n×4 / n×(4*(K-1)) を許容)
  get_init4 <- function(i_row, Lidx, is_ref_row) {
    if (is.null(initial.CIFs)) return(exp(log_p0))  # length 4

    nc <- ncol(initial.CIFs)
    if (nc == 2L) {
      v2 <- clampP(as.numeric(initial.CIFs[i_row, 1:2, drop = TRUE]), prob.bound)
      return(c(v2[1], v2[2], v2[1], v2[2]))
    }
    if (nc == 4L) {
      return(as.numeric(initial.CIFs[i_row, 1:4, drop = TRUE]))
    }
    if (nc == 4L * K_nonref) {
      off <- 4L * (Lidx - 1L)
      return(as.numeric(initial.CIFs[i_row, off + (1:4), drop = TRUE]))
    }
    stop(sprintf("initial.CIFs は 2 列, 4 列, または %d 列である必要があります（got %d）。",
                 4L * K_nonref, nc))
  }
  ...
  for (i in seq_len(nrow(x_l))) {
    Lidx <- if (lvl_idx[i] == 0L) anchor_idx else lvl_idx[i]
    if (Lidx < 1L || Lidx > K_nonref) {
      warning(sprintf("行 %d: Lidx=%d が範囲外(1..%d)。この行は参照フォールバックを使用。", i, Lidx, K_nonref))
      predicted.CIFs[i, ] <- p0_ref
      next
    }

    b1 <- beta_tmp_1[Lidx]; b2 <- beta_tmp_2[Lidx]
    a1 <- alpha_tmp_1[i];    a2 <- alpha_tmp_2[i]
    key <- key_of(a1, a2, Lidx)

    # 解の取得（キャッシュ + try/fallback）
    got <- TRUE
    log_CIFs <- tryCatch({
      if (exists(key, envir = cache, inherits = FALSE)) {
        get(key, envir = cache)
      } else {
        CIFs0     <- clampP(get_init4(i, Lidx, lvl_idx[i] == 0L), prob.bound)
        log_CIFs0 <- log(CIFs0)
        ans <- callLevenbergMarquardt(log_CIFs0, a1, b1, a2, b2, optim.method, prob.bound, estimand)
        print("2")
        if (any(!is.finite(ans))) stop("non-finite solution")
        assign(key, ans, envir = cache); ans
      }
    }, error = function(e) { got <<- FALSE; NA_real_ })

    if (!got) {
      fail_rows <- c(fail_rows, i)
      predicted.CIFs[i, ] <- if (lvl_idx[i] == 0L) p0_ref else p0_non
      next
    }
    predicted.CIFs[i, ] <- clampP(exp(log_CIFs), prob.bound)
  }

  if (length(fail_rows) > 0L && isTRUE(getOption("polyreg.warn.lm_fallback", TRUE))) {
    warning(sprintf("Levenberg-Marquardt の解が得られず %d 行でフォールバックを使用（例: 行 %s）。",
                    length(fail_rows), paste(utils::head(fail_rows, 5L), collapse = ",")))
  }
  return(predicted.CIFs)
}
...
residuals_CIFs <- function(log_p, alpha1, beta1, alpha2, beta2, prob.bound, estimand) {
...
jacobian_CIFs <- function(log_p, alpha1, beta1, alpha2, beta2, effect.measure1, effect.measure2, prob.bound) {
  clogp <- clampLogP(as.numeric(log_p))
  m <- length(clogp); if (m %% 2L != 0L) stop("log_p length must be even (2K).")
  K <- m %/% 2L
  i1 <- 1:K; i2 <- (K+1):(2*K)
  ...
  row_e1 <- 2*K - 1
  row_e2 <- 2*K
  add_effect_grad(estimand$effect.measure1, pref = p1[1], pL = p1[2:K], row = row_e1, idx_ref = i1[1], idx_L = i1[2:K])
  add_effect_grad(estimand$effect.measure2, pref = p2[1], pL = p2[2:K], row = row_e2, idx_ref = i2[1], idx_L = i2[2:K])
  J
}

callLevenbergMarquardt <- function(log_CIFs0, alpha1, beta1, alpha2, beta2, optim.method, prob.bound, estimand) {
  res_fun <- function(lp) residuals_CIFs(lp, alpha1, beta1, alpha2, beta2, prob.bound, estimand)
  jac_fun <- function(lp) jacobian_CIFs(lp, alpha1, beta1, alpha2, beta2, prob.bound, estimand)
  LevenbergMarquardt(
    start         = log_CIFs0,
    res_fun       = res_fun,
    jac_fun       = jac_fun,
...
chooseEffectMeasure <- function(effect.measure, prob.bound) {
  if (effect.measure == "RR") {
    f  <- function(p, beta) { p * exp(beta) }
    df <- function(p, beta) { exp(beta) }
    ...
    stop("Invalid effect measure. Use RR, OR, or SHR.")
  }
  list(f = f, df = df)
}

residuals_CIFs <- function(log_p, alpha1, beta1, alpha2, beta2, prob.bound, cem1, cem2) {
  clogp <- clampLogP(as.numeric(log_p))
  if (length(clogp) < 2L) clogp <- rep(clogp, 2L)

  p1 <- exp(clogp[1])
  p2 <- exp(clogp[2])
  p3 <- clampP(cem1$f(p1, beta1), prob.bound)
  p4 <- clampP(cem2$f(p2, beta2), prob.bound)

  rem12 <- max(1 - p1 - p2, prob.bound)
  rem34 <- max(1 - p3 - p4, prob.bound)
  lp0102 <- log(rem12) + log(rem34)

  c(alpha1 - log(p1) - log(p3) + lp0102,
    alpha2 - log(p2) - log(p4) + lp0102)
}

jacobian_CIFs <- function(log_p, alpha1, beta1, alpha2, beta2, prob.bound, cem1, cem2) {
  clogp <- clampLogP(as.numeric(log_p))
  if (length(clogp) < 2L) clogp <- rep(clogp, 2L)
  p1 <- exp(clogp[1]); p2 <- exp(clogp[2])
  ...
  dp3_dlp1 <- cem1$df(p1, beta1) * p1
  dp4_dlp2 <- cem2$df(p2, beta2) * p2

  dlogBi_dlp1 <- (-p1) / rem12
  dlogBi_dlp2 <- (-p2) / rem12
  dlogBj_dlp1 <- ( -dp3_dlp1) / rem34
  dlogBj_dlp2 <- ( -dp4_dlp2) / rem34

  J <- matrix(0.0, 2L, 2L)
  J[1, 1] <- -1 - (dp3_dlp1 / p3) + dlogBi_dlp1 + dlogBj_dlp1
  J[1, 2] <-                    0 + dlogBi_dlp2 + dlogBj_dlp2
  J[2, 1] <-                    0 + dlogBi_dlp1 + dlogBj_dlp1
  J[2, 2] <- -1 - (dp4_dlp2 / p4) + dlogBi_dlp2 + dlogBj_dlp2
  J
}

callLevenbergMarquardt_new <- function(log_CIFs0, alpha1, beta1, alpha2, beta2,
                                   optim.method, prob.bound, cem1, cem2) {
  res_fun <- function(lp) residuals_CIFs(lp, alpha1, beta1, alpha2, beta2, prob.bound, cem1, cem2)
  jac_fun <- function(lp) jacobian_CIFs(lp, alpha1, beta1, alpha2, beta2, prob.bound, cem1, cem2)
  LevenbergMarquardt(
    start         = as.numeric(log_CIFs0)[1:2],
    res_fun       = res_fun,
    jac_fun       = jac_fun,
    maxit         = optim.method$optim.parameter6   %||% 30,
    ftol          = optim.method$optim.parameter7   %||% 1e-10,
...
callLevenbergMarquardt_old <- function(log_CIFs0, alpha1, beta1, alpha2, beta2, optim.method, estimand, prob.bound, cemlm1, cemlm2)
{
